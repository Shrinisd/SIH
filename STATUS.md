...
| **End-to-end latency on real hardware** (the "Low Latency" half of the problem statement) | **Not yet measured — needs hardware** | The only real number so far is model `Invoke()` time on a laptop CPU (~1–4ms, via `scripts/live_mic_demo.py`), which is not representative of the ESP32-S3. An older estimate in `ml/results/physical_latency_report.json` targets a different board (Pico 2W) and was never hardware-validated even there — see the correction in `ml/results/README.md`. The firmware already times every inference with `esp_timer_get_time()` and logs it live over serial (`firmware/src/main.cpp`), so a real, trustworthy number appears automatically the moment hardware is flashed — this should be one of the first things captured and written down once the board arrives |
